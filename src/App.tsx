  ...
  return (
    <div className="min-h-screen">
        ...
        {showInstallBanner && (
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-black uppercase tracking-tight">Enjoy Offline?</p>
                <p className="text-[10px] font-bold opacity-70">Install Kobir Aesthetics</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button 
        ...
        {selectedPhoto && (
          ...
          >
            ...
            <div className="w-full h-full relative overflow-hidden flex flex-col">
              ...
              <div className="flex items-center justify-between p-6 bg-gradient-to-b from-black/60 to-transparent sticky top-0 z-[100] w-full">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full border border-white/20 p-0.5">
                    <img src="/favicon-96x96.png" className="w-full h-full rounded-full object-contain brightness-110" alt="Logo" />
                  </div>
                  <div>
                    <h4 className="text-sm font-black tracking-tight text-white uppercase">Kobir Aesthetics</h4>
                    <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{selectedPhoto.section.replace(/_/g, ' ')}</p>
                  </div>
                </div>
              ...
              <div className="flex-1 relative flex items-center justify-center overflow-hidden touch-none -mt-20">
                ...
                >
                  <motion.img
                    ...
                    className="max-w-full max-h-screen object-contain shadow-[0_50px_100px_rgba(0,0,0,0.5)]"
                  />
                </motion.div>
              </div>
              ...
              <motion.div 
                ...
                className="absolute bottom-0 left-0 right-0 z-[90] bg-[#121212] border-t border-white/10 rounded-t-[3rem] shadow-[0_-20px_60px_rgba(0,0,0,0.8)] backdrop-blur-3xl min-h-[80%] overflow-y-auto cursor-grab active:cursor-grabbing pb-20"
              >
                {/* Drag Handle & Visual Hint */}
                <div className="sticky top-0 z-[100] bg-[#121212]/50 backdrop-blur-md pb-2">
                  <div className="w-12 h-1.5 bg-white/20 rounded-full mx-auto my-6" />
                  {!isSheetOpen && (
                    <div className="flex justify-center mb-2">
                      <p className="text-[10px] uppercase font-black text-white/40 tracking-[0.2em] animate-bounce">Details</p>
                    </div>
                  )}
                </div>
                
                <div className="max-w-xl mx-auto px-8 pb-12 text-white">
                  <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 bg-white/5 border border-white/5 rounded-[2rem] text-center">
                        <p className="text-[10px] uppercase text-white/40 font-black mb-1">Channel</p>
                        <p className="text-sm font-bold text-white">{selectedPhoto.section.replace(/_/g, ' ')}</p>
                      </div>
                      <div className="p-4 bg-white/5 border border-white/5 rounded-[2rem] text-center">
                        <p className="text-[10px] uppercase text-white/40 font-black mb-1">Resolution</p>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        className="fixed bottom-8 right-8 w-16 h-16 bg-[var(--primary)] text-black rounded-full flex items-center justify-center shadow-2xl z-40 md:hidden"
      >
        <Filter size={24} />
      </motion.button>

      {/* Footer Info */}
      <footer className="py-12 px-4 text-center border-t border-[var(--glass-border)] mt-20">
         <div className="flex items-center justify-center gap-2 mb-4">
            <img src="/favicon-96x96.png" className="w-8 h-8 opacity-20 contrast-100" alt="" />
            <p className="text-[var(--text-dim)]/40 font-black tracking-[0.2em] text-xs uppercase">Kobir Lyrics Studio</p>
         </div>
         <p className="text-[var(--text-dim)]/30 text-[10px] max-w-sm mx-auto">
            All content on Kobir Lyrics is free for personal use. Handcrafted with obsession by the Gopal's Diary team.
         </p>
      </footer>
    </div>
  );
